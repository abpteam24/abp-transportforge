	...
	if ( ! class_exists( 'ABPTF_Status' ) ) {
		class ABPTF_Status {
			...
			public function create_page(): void {
				if ( ! check_ajax_referer( 'abptf_admin_ajax_nonce', 'nonce', false ) ) {
					wp_send_json_error( [ 'info_type' => 'warn', 'msg' => __( 'Invalid security token.', 'abp-transportforge' ) ], 403 );
				}
				if ( ! current_user_can( 'manage_options' ) ) {
					wp_send_json_error( [ 'info_type' => 'warn', 'msg' => __( 'Insufficient permissions.', 'abp-transportforge' ) ], 403 );
				}
			...
			public static function array_info( $key ) {
				...
				$des          = array(
					...
					//=============================//
					'route_config_1'              => __( 'Note: Configure the transport route by selecting the stops and their types.Here you can also add new stops ', 'abp-transportforge' ),
					'route_config_2'              => __( 'Note: Boarding means passengers can only board at that stop, Dropping means passengers can only get off, and Both allows both boarding and dropping. The first stop must always be Boarding, and the last stop must always be Dropping. ', 'abp-transportforge' ),
					'route_config_3'              => __( 'Note: The first stop time must be 0 minutes. All other stop times represent the travel time in minutes from the first stop. These times are applied based on your Transport Time Configuration. ', 'abp-transportforge' ),
					'route_config_4'              => __( 'Note: You can also enable Multiple Pickup Points for boarding stops. The available pickup points are loaded from the selected stop, and their time offsets are applied automatically. Likewise, you can enable Multiple Drop Points for dropping stops. Stops with the Both type support both multiple pickup and multiple drop points. ', 'abp-transportforge' ),
					//=============================//
