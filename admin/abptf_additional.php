	...
	if ( ! class_exists( 'ABPTF_Additional' ) ) {
		class ABPTF_Additional {
			public function __construct() {
				...
				add_action( 'wp_ajax_abptf_save_additional_service', array( $this, 'save_global_additional_service' ) );
			...
			public function post_additional_service( $abptf_infos ): void {
				if ( ABPTF_Function::on_off( 'additional_info' ) ) {
					$additional_services      = $abptf_infos['additional_services'] ?? array();
					$display                  = $abptf_infos['display_additional_services'] ?? 'on';
					$active_global_additional = $abptf_infos['active_global_additional'] ?? 'on';
					?>
                ...
                <div class="configuration_content additional_service">
                    ...
                    <div class="_fj_between">
						<?php ABPTF_Layout::button_add_xs( __( 'Add New Additional services', 'abp-transportforge' ) );
							if ( $global ) {
								ABPTF_Layout::button_save_xs( __( 'Save Global Additional services Configuration', 'abp-transportforge' ) );
							} ?>
                    </div>
			...
			public function service_item( $key = '', $field = array() ): void {
                ...
                <tr class="delete_area ">
                    <td> <?php do_action( 'abptf_add_image_icon', 'additional_icon[]', $icon_image ); ?>  </td>
                    <td>
			...
			public function save_global_additional_service(): void {
				...
				wp_send_json_success( [ 'msg' => __( 'Additional services Configuration Saved Successfully ..... !! ', 'abp-transportforge' ) ] );
			}
